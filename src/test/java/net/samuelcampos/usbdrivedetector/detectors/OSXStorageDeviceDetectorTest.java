...
import net.samuelcampos.usbdrivedetector.process.FileOutputProcessor;
...
public class OSXStorageDeviceDetectorTest {
    ...
    public void testGetStorageDevices() throws IOException {
        ...
        when(commandExecutor.executeCommand("df -l"))
                .thenAnswer(ignore -> new FileOutputProcessor(TestUtils.getFileFromClassPath("macos/df-l.txt")));

        when(commandExecutor.executeCommand(argThat(new StartsWith("diskutil info "))))
                .thenAnswer(ignore -> new FileOutputProcessor(TestUtils.getFileFromClassPath("macos/diskutil-non-usb.txt")));
